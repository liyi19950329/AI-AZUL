...
class myPlayer(AdvancePlayer):
    ...
    def StartRound(self, game_state):
        ...
        for move in moves:
            ...
            recentscore = self.pureMINIMAX(moveState, 2, 1 - self.id)
    ...
    def pureMINIMAX(self, gameState, depth, playerId):
        ...
        if playerId == self.id:
            ...
            for move in moves:
                if self.getScore(move, gameState, playerId) > 1:
                    ...
                    recentValue = self.pureMINIMAX(moveState, depth - 1, 1 - playerId)
    ...
    def SelectMove(self, moves, game_state):
        ...
        if not self.bestbestMove:
            ...
            for move in moves:
                moveState = copy.deepcopy(game_state)
                moveState.ExecuteMove(self.id, move)
                recentscore = self.pureMINIMAX(moveState, 1, 1 - self.id)
                if recentscore > bestscore:
                    bestscore = recentscore
                    best_move = move
