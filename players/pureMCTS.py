...
class myPlayer(AdvancePlayer):
    ...
    def MCTS(self, gameState, times, deep):
        ...
        for i in range(times):
            ...
            curr_stateB = copy.deepcopy(gameState.players[1 - self.id])
            ...
            for j in range(deep):
                if moves and (j < deep - 1):
                    ...
                    if len(moves) >= 1:
                        ...
                        reward += comboNumber * comboReward * (lineNum - 2) + completeReward + lineReward * (
                                    (num_to_pattern_line / lineNum) - 0.5) + helpCompleteReward + central_reward
                        # 后面行，每次放的越多，reward越高
                        ...
                        # 更新当前状态
                        curr_stateB = copy.deepcopy(next_gameState.players[1 - self.id])
                    ...
                    policyvalu = (reward + cost) * 0.3
                    score = score_change - score_changeB
    ...
    def SelectMove(self, moves, game_state):
        ...
        gs_copy = copy.deepcopy(game_state)
        plystate = copy.deepcopy(gs_copy.players[self.id])
        curr_state = PlayerToString(self.id, plystate)
        best_move = None

        maxValue = -100

        self.MCTS(gs_copy, 50, 2)
